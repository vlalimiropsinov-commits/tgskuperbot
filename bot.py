...
def get_completed_applications():
    applications = get_applications()
    return [app for app in applications if app.get('status') == 'completed']
...
def calculate_income(period='day'):
    applications = get_completed_applications()
    now = datetime.now()
    total_income = 0
    
    for app in applications:
        completed_time = datetime.fromisoformat(app.get('completed_at', now.isoformat()))
        
        if period == 'day' and completed_time.date() == now.date():
            total_income += 1.3 * app.get('account_count', 1)
        elif period == 'week' and completed_time >= now - timedelta(days=7):
            total_income += 1.3 * app.get('account_count', 1)
        elif period == 'month' and completed_time >= now - timedelta(days=30):
            total_income += 1.3 * app.get('account_count', 1)
        elif period == 'year' and completed_time >= now - timedelta(days=365):
            total_income += 1.3 * app.get('account_count', 1)
        elif period == 'all':
            total_income += 1.3 * app.get('account_count', 1)
...
def send_welcome(message):
    ...
    if is_admin(message.from_user.id):
        ...
        if is_user_in_active_chat(message.from_user.id):
            ...
            welcome_text = """
🤖 <b>Добро пожаловать в бота по скупке Telegram аккаунтов!</b>

🚀 <b>Быстро • Выгодно • Надежно</b>

💰 <b>Стоимость:</b> $1.3 за аккаунт
💵 <b>Выплата:</b> CryptoBot / Крипто-кошелек
...
# ОБЩИЙ ДОХОД
@bot.message_handler(func=lambda message: message.text == '💰 Общий доход' and (is_admin(message.from_user.id) or is_report_user(message.from_user.id)))
def show_income(message):
    ...
    completed_deals = len(get_completed_applications())
    pending_deals = len(get_pending_applications())
...
def handle_broadcast_confirmation(call):
    if call.data == 'cancel_broadcast':
        ...
        # Получаем всех пользователей из завершенных сделок
        applications = get_completed_applications()
        user_ids = list(set([app['user_id'] for app in applications]))
...
def handle_complete_deal(call):
    ...
    update_application_status(user_id, 'completed')
    
    # Закрываем активный чат
    close_active_chat(user_id)
    
    # Находим данные сделки для отчета
    applications = get_completed_applications()
    deal_data = None
    for app in applications:
        if app.get('user_id') == user_id:
...
def start_sell_process(message):
    ...
    msg = bot.send_message(message.chat.id, "🔢 Сколько аккаунтов хотите продать? (1-25)\n\nДля отмены используйте /cancel")
...
def process_account_count(message):
    ...
    try:
        ...
        if account_count == 1:
            msg = bot.send_message(message.chat.id, "📞 Введите номер телефона аккаунта:\n\nДля отмены используйте /cancel")
            bot.register_next_step_handler(msg, process_single_phone, user_data)
        else:
            msg = bot.send_message(message.chat.id, f"📞 Введите {account_count} номеров телефонов через запятую:\n\nДля отмены используйте /cancel")
...
def handle_all_messages(message):
    ...
    if is_admin(message.from_user.id) and message.text not in ['📋 Список заявок', '📢 Сделать рассылку', '💰 Общий доход']:
    ...
    if not is_admin(message.from_user.id) and message.text not in ['💰 Продать аккаунт', 'ℹ️ Информация'] and not is_user_in_active_chat(message.from_user.id):
